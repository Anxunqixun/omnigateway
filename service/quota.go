...
func PostWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelName string,
	...
	wssParams := billingexpr.TokenParams{
		P:   float64(usage.InputTokens),
		C:   float64(usage.OutputTokens),
		Len: float64(usage.InputTokens),
	}
	tieredOk, tieredQuota, tieredRes := TryTieredSettle(relayInfo, wssParams)
	if tieredOk {
	...
	wssCost := AttachDualLedger(other, relayInfo, logModel, quota, wssParams)
...
func PostAudioConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent string) {
	...
	var tieredResult *billingexpr.TieredResult
	AttachSettledUsageView(relayInfo, usage)
	audioParams := BuildTieredTokenParams(usage, false, tieredUsedVars)
	tieredOk, tieredQuota, tieredRes := TryTieredSettle(relayInfo, audioParams)
	if tieredOk {
		tieredResult = tieredRes
	}
	...
	audioCost := AttachDualLedger(other, relayInfo, logModel, quota, audioParams)
	attachQuotaSaturation(ctx, relayInfo, other)
	model.RecordConsumeLog(ctx, relayInfo.UserId, model.RecordConsumeLogParams{
		ChannelId:        relayInfo.ChannelId,
