...
func TryTieredSettle(relayInfo *relaycommon.RelayInfo, params billingexpr.TokenParams) (ok bool, quota int, result *billingexpr.TieredResult) {
	...
	if err != nil {
		// v1 keeps historical fallback so official models stay compatible.
		// v2 fail-closed: keep the already reserved pre-consume, do not invent a price.
		quota = relayInfo.FinalPreConsumedQuota
		if quota <= 0 {
			quota = snap.EstimatedQuotaAfterGroup
		}
		if snap.ExprVersion >= billingexpr.ExprVersionV2 && relayInfo != nil {
			relayInfo.BillingSettleError = err.Error()
		}
