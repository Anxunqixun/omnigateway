...
type Model struct {
	...
	DocsMarkdown  string         `json:"docs_markdown,omitempty" gorm:"type:text"`
	DocsPublished bool           `json:"docs_published"`
	DocsTemplate  string         `json:"docs_template,omitempty" gorm:"type:varchar(32)"`
...
func (mi *Model) Update() error {
	...
	return DB.Model(&Model{}).Where("id = ?", mi.Id).
		Select("model_name", "description", "icon", "tags", "vendor_id", "endpoints", "status", "sync_official", "name_rule", "docs_markdown", "docs_published", "docs_template", "updated_time").
		Updates(mi).Error
}
...
func ListPublishedModelDocs() ([]Model, error) {
	var models []Model
	err := DB.Where("docs_published = ? AND docs_markdown <> ? AND status = ?", true, "", 1).
		Order("model_name").
		Find(&models).Error
	return models, err
}

func GetPublishedModelDoc(name string) (*Model, error) {
	var m Model
	err := DB.Where("model_name = ? AND docs_published = ? AND docs_markdown <> ?", name, true, "").
		First(&m).Error
	if err != nil {
		return nil, err
	}
