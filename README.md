...
基于 [QuantumNous/new-api](https://github.com/QuantumNous/new-api) **v1.0.0-rc.26** 的二次开发。

上游品牌、版权、许可证与署名保持不变（AGPLv3）。  
官方完整说明：[README.upstream.md](./README.upstream.md) · [简体中文官方说明](./README.zh_CN.md)

</div>

本仓库不要把生产镜像跟踪为 `calciumion/new-api:latest`。升级时在锁定标签上 cherry-pick 官方补丁，再重新构建本仓库镜像。

更细的计费表达式语法见 [`pkg/billingexpr/expr.md`](./pkg/billingexpr/expr.md)。官方基线见 [`docs/omnigateway/BASELINE.md`](./docs/omnigateway/BASELINE.md)。
...
Modified versions must preserve the author attribution notice `Frontend design and development by New API contributors.` and a visible link to the original project: https://github.com/QuantumNous/new-api
