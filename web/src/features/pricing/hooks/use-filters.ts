...
export function useFilters(models: PricingModel[], defaultGroup?: string) {
  ...
  const appliedDefaultGroup = useRef(false)

  useEffect(() => {
    if (appliedDefaultGroup.current || search.group || !defaultGroup) {
      return
    }
    appliedDefaultGroup.current = true
    setFilterState((prev) =>
      prev.group ? prev : { ...prev, group: defaultGroup }
    )
  }, [defaultGroup, search.group])
